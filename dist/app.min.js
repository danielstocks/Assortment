...
!function(n){function t(n,t){for(var e=[],c=0,l=0;c<n.length&&l<t.length;)n[c]<t[l]?e.push(n[c++]):e.push(t[l++]);return e.concat(n.slice(c)).concat(t.slice(l))}function e(n){if(n.length<2)return n;var c=Math.floor(n.length/2),l=n.slice(0,c),r=n.slice(c),i=t(e(l),e(r));return i.unshift(0,n.length),n.splice.apply(n,i),n}n.mergeSort=e}(window);
...
!function(n){function s(n){var s,r=n.length;for(i=0;i<r;i++){for(s=i,j=i+1;j<r;j++)n[j]<n[s]&&(s=j,swaps.push([n.slice(0),j]));i!=s&&o(n,i,s)}return n}function o(i,n,s){var o=i[n];i[n]=i[s],i[s]=o,swaps.push([i.slice(0),s,n])}n.selectionSort=s}(window);
...
!function(n){function t(n,t,e){var i=u.createGain(),o=u.createPanner(),s=u.createOscillator();return e?o.setPosition(10,5,0):o.setPosition(-10,-5,0),i.connect(o),o.connect(u.destination),i.gain.value=0,s.type=t,s.frequency.value=n,s.connect(i),s.start(u.currentTime),i.gain.value=a,s.stop(u.currentTime+.5),setTimeout(function(){i.gain.value=0},300),s}window.AudioContext=window.AudioContext||window.webkitAudioContext;var e={},i=175,o=600,u=new AudioContext,a=.5;e.draw=function(n){var e=(o-i)/currentList.length,u=swaps[n][1]*e+i,a=swaps[n][2]*e+i;isFinite(u)&&t(u,"triangle",!0),isFinite(a)&&t(a,"square")},e.mute=function(){a=0},e.unmute=function(){a=.5},state.on("sound-mute",e.mute),state.on("sound-unmute",e.unmute),n.sound=e}(window);
...
!function(t){var n={randomize:document.getElementById("randomize"),play:document.getElementById("play"),mute:document.getElementById("mute"),items:document.getElementById("num-items")};n.mute.onclick=function(t){t.preventDefault(),"mute"==this.innerHTML.trim()?state.trigger("sound-mute"):state.trigger("sound-unmute")},n.items.onchange=function(){state.trigger("animation-stop"),currentList=randomArray(),state.trigger("new-list"),chart.draw()},n.randomize.onclick=function(t){t.preventDefault(),state.trigger("animation-stop"),currentList=randomArray(),state.trigger("new-list"),chart.draw()},sorted=!1,n.play.onclick=function(t){return t.preventDefault(),"pause"==this.innerHTML.trim()?state.trigger("animation-pause"):(sorted||(swaps=[],sort(currentList),sorted=!0),void state.trigger("animation-play",[swaps]))},state.on("new-list",function(){sorted=!1}),state.on("animation-end",function(){n.play.innerHTML="replay"}),state.on("animation-stop",function(){n.play.innerHTML="play"}),state.on("animation-pause",function(){n.play.innerHTML="play"}),state.on("animation-play",function(){n.play.innerHTML="pause"}),state.on("sound-mute",function(){n.mute.innerHTML="unmute"}),state.on("sound-unmute",function(){n.mute.innerHTML="mute"})}(window);
!function(n){function t(){u=0}var a,e,o,i={},u=0,s=60,r=Date.now(),m=1e3/s;document.getElementById("fps").onchange=function(){s=parseInt(this.value),m=1e3/s},i.start=function(n){sound.unmute(),function i(s){if(a=window.requestAnimationFrame(i),e=Date.now(),o=e-r,o>m){if(r=e-o%m,!(u<n.length))return chart.draw(u),sound.mute(),state.trigger("animation-stop"),state.trigger("animation-end"),void t();currentList=n[u][0],chart.draw(u),sound.draw(u),u++}}()},i.pause=function(){window.cancelAnimationFrame(a),sound.mute()},i.stop=function(){sound.mute(),window.cancelAnimationFrame(a),a=null,u=0},state.on("animation-pause",i.pause),state.on("animation-play",i.start),state.on("animation-stop",i.stop)}(window);